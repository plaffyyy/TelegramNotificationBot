    ...
    public String parse(ObjectNode response, ObjectNode lastUpdate) {
        ...
        if (newPulls.size() > lastPulls.size()) {
            ...
            result.append("Новый пул реквест:\n")
                ...
                .append("Превью описания: ").append(newPull.get("body").asText().substring(0, Math.min(200, newPull.get("body").asText().length()))).append("\n");
        }
        ...
        if (newIssues.size() > lastIssues.size()) {
            ...
            result.append("Новое Issue:\n")
                ...
                .append("Превью описания: ").append(newIssue.get("body").asText().substring(0, Math.min(200, newIssue.get("body").asText().length()))).append("\n");        }
